...
@section('content')
    ...
    <form action="{{ route('edit-tour/' . $detail->id) }}" method="post" enctype="multipart/form-data">
        {{-- tên tour --}}
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" name="description" rows="3" required>{{ $detail->description }}</textarea>
        </div>

        {{-- giá tour --}}
        <div class="mb-3">
            <label for="price" class="form-label">Giá tour</label>
            <input type="number" class="form-control" name="price" step="0.01" value="{{ $detail->price }}" required>
        </div>
        ...
        <div class="mb-3">
            ...
            @if($detail->thumbnail)
                <img src="{{ $detail->thumbnail }}" alt="thumbnail" width="100">
            @endif
        </div>
